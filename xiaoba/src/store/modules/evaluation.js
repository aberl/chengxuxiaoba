...
const actions = {
  ...
  async getAllEvaluationList({ commit },  {videoId, pageNum, pagesize}) {
    const result = await reqGetAllEvaluatesList(videoId,pageNum,pagesize,"id");
    if (result.code == 0) {
      commit(REQUEST_RECEIVE_EVALUATIONALLLIST, { evaluationList: result.data });
    }
  },
  async getAllUserEvaluationList({ commit },  {userId, pageNum, pagesize}) {
    const result = await reqGetUserAllEvaluatesList(userId,pageNum,pagesize,"id");
    if (result.code == 0) {
      commit(REQUEST_RECEIVE_EVALUATIONALLLIST, { evaluationList: result.data });
    }
  },
