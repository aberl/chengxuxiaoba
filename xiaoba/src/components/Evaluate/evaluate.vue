<template>
  <div>
    ...
    <div class="p-3">
      <small v-if="!this.hasBeenSubmitEvaluation" class="d-block text-right border-bottom padding-bottom">
        <el-button
      ...
      <small v-else class="d-block text-right border-bottom padding-bottom">
        <el-button type="primary" round  disabled>您已评价</el-button>
      </small>
      <div v-for="evaluation in evaluationList" :key="evaluation.id">
        <div class="media padding-top">
          ...
          <p class="media-body text-left">
            <strong class="d-block">{{evaluation.accountName}}<p v-if="evaluation.accountName == userInfo.name" class="el-icon-s-custom"><p/></strong>
...
export default {
  ...
  computed: {
    hasBeenSubmitEvaluation:function(){
      let currentName=this.userInfo.name;
      let filter = this.evaluationList.find(function(val){
        return val.accountName==currentName;
      });
